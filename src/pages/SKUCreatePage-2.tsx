...
const SKUCreatePage: React.FC = () => {
  ...
  const handleChange = (field: keyof FormData, value: any) => {
    // Enforce max length for model number
    if (field === 'model' && value.length > 20) {
  ...
  const validateForm = (): boolean => {
    ...
    // Model Number validation (required, max 20 characters)
    if (!validateRequired(formData.model)) {
      newErrors.model = 'Model Number is required';
    } else if (formData.model.length > 20) {
      newErrors.model = 'Model Number must be maximum 20 characters';
    }
