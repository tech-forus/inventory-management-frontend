...
const OnboardingWizard: React.FC = () => {
  ...
  const loadProductCategories = async (cid: string) => {
    try {
      const response = await api.get(`/onboarding/product-categories/${cid}`);
  ...
  const loadItemCategories = async (cid: string, productCategoryId: number) => {
    try {
      const response = await api.get(`/onboarding/item-categories/${cid}/${productCategoryId}`);
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1) {
      ...
      try {
        await api.post('/onboarding/product-categories', {
          companyId,
        ...
        setTimeout(async () => {
          const response = await api.get(`/onboarding/product-categories/${companyId}`);
          if (response.data.success && response.data.data.length > 0) {
      ...
      if (selectedProductCategory && itemCategories.length > 0) {
        ...
        try {
          await api.post('/onboarding/item-categories', {
            companyId,
      ...
      if (selectedItemCategory && subCategories.length > 0) {
        // Save sub categories
        try {
          await api.post('/onboarding/sub-categories', {
            companyId,
  ...
  const handleComplete = async () => {
    ...
    try {
      // Save vendors
      if (vendors.length > 0) {
        await api.post('/onboarding/vendors', {
          companyId,
      ...
      // Save brands
      if (brands.length > 0) {
        await api.post('/onboarding/brands', {
          companyId,
  ...
  const removeProductCategory = async (index: number, categoryId?: number) => {
    ...
    if (categoryId) {
      try {
        await api.delete(`/onboarding/product-categories/${categoryId}`);
        await loadProductCategories(companyId);
      } catch (error) {
  ...
  const removeItemCategory = async (index: number, categoryId?: number) => {
    ...
    if (categoryId && selectedProductCategory) {
      try {
        await api.delete(`/onboarding/item-categories/${categoryId}`);
        await loadItemCategories(companyId, selectedProductCategory);
      } catch (error) {
  ...
  const removeSubCategory = async (index: number, categoryId?: number) => {
    ...
    if (categoryId && selectedItemCategory) {
      try {
        await api.delete(`/onboarding/sub-categories/${categoryId}`);
  ...
  const removeVendor = async (index: number, vendorId?: number) => {
    ...
    if (vendorId) {
      try {
        await api.delete(`/onboarding/vendors/${vendorId}`);
  ...
  const removeBrand = async (index: number, brandId?: number) => {
    ...
    if (brandId) {
      try {
        await api.delete(`/onboarding/brands/${brandId}`);
