...
const CategoryMasterTab: React.FC<CategoryMasterTabProps> = ({
  ...
  const handleDownloadTemplate = () => {
    // Create unified template with three sheets
    const wb = XLSX.utils.book_new();
    
    // Product Categories Sheet
    const productData = [{ 'Name': '' }];
    const productWs = XLSX.utils.json_to_sheet(productData);
    productWs['!cols'] = [{ wch: 30 }];
    XLSX.utils.book_append_sheet(wb, productWs, 'Product Categories');
    
    // Item Categories Sheet
    const itemData = [{ 'Product Category': '', 'Name': '' }];
    const itemWs = XLSX.utils.json_to_sheet(itemData);
    itemWs['!cols'] = [{ wch: 25 }, { wch: 30 }];
    XLSX.utils.book_append_sheet(wb, itemWs, 'Item Categories');
    
    // Sub Categories Sheet
    const subData = [{ 'Item Category': '', 'Name': '' }];
    const subWs = XLSX.utils.json_to_sheet(subData);
    subWs['!cols'] = [{ wch: 25 }, { wch: 30 }];
    XLSX.utils.book_append_sheet(wb, subWs, 'Sub Categories');
  ...
  const handleFileUpload = async (file: File | null) => {
    ...
    try {
      setSaving(true);
      const result = await libraryService.uploadCategories(file);
