...
const ItemCategoriesTab: React.FC<ItemCategoriesTabProps> = ({
  itemCategories,
  loading,
  onRefresh,
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Are you sure you want to delete this item category?')) return;

    try {
      setSaving(true);
      await libraryService.deleteYourItemCategory(id);
      onRefresh();
    } catch (error: any) {
      alert(error.response?.data?.error || 'Failed to delete item category');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleFileUpload = async (file: File | null) => {
    ...
    try {
      setSaving(true);
      const result = await libraryService.uploadItemCategories(file);
      if (result.success) {
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Filter by Product Category</label>
          <select
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Item Category Name</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Product Category</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Sub Category Count</th>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    value={categoryForm.productCategoryId}
                    onChange={(e) => setCategoryForm({ ...categoryForm, productCategoryId: parseInt(e.target.value) })}
                        ...
                        <div key={index} className="flex gap-2 items-center p-3 bg-gray-50 rounded-lg border border-gray-200">
                          <div className="flex-1">
                            <input
                              ...
                              placeholder={`Item Category Name ${index + 1}`}
