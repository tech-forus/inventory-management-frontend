...
const ProductCategoriesTab: React.FC<ProductCategoriesTabProps> = ({ productCategories, loading, onRefresh }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (editingCategory) {
        await libraryService.updateYourProductCategory(editingCategory.id, categoryForm);
      } else {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      setSaving(true);
      await libraryService.deleteYourProductCategory(id);
      onRefresh();
    } catch (error: any) {
      alert(error.response?.data?.error || 'Failed to delete product category');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleFileUpload = async (file: File | null) => {
    ...
    try {
      setSaving(true);
      const result = await libraryService.uploadProductCategories(file);
      if (result.success) {
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Category Name</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Description</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Item Count</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredCategories.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-gray-500">No product categories found</td>
                    </tr>
                  ) : (
                    filteredCategories.map((category) => (
                      <tr key={category.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-700">{category.itemCount || 0}</td>
